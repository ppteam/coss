...
function CenterContain() {
	var contain = null;
	var _statusbar = null;
	var projPerson_form = null;
	var projPerson_dialog = null;
	var start = null;
	var end = null;
	...
	function buildContain() {
		projPerson_form = new Ext.FormPanel({
					region : 'center',
					labelWidth : 53,
					frame : true,
		...
		_dataModel.initDeptGroup(projPerson_form);
		projPerson_dialog = new Ext.Panel({
					title : '项目-人员工作量统计报表',
					layout : 'border',
					region : 'center',
					items : [projPerson_form]
				});
		contain = new Ext.Panel({
					layout : 'border',
					region : 'center',
					bbar : _statusbar,
					items : [projPerson_dialog]
				});
	...
	function exportReport() {
		...
		if (_arg_ids.length === 0) {
			...
			var _title = 'PROJECT_BY_WORKER_TOTAL';
			window.location.href = 'http://' + window.location.host
					+ '/workbase/report/proxy/view.xls?view=ProjectStaff&startDate=' + start + '&endDate=' + end
