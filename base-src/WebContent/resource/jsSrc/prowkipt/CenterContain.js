...
function CenterContain() {
											...
											items : [{
														...
														listeners : {
															change : function(dateField, newValue, oldValue) {
																model.setSelectedDate(newValue);
															} // end_change
														}
													}]
										}, {
	...
	function cardNav(/* int */incr,/* Button */prev,/* Button */next) {
		...
		if (n === 1 && incr > 0) {
			if (edit_form.getForm().isValid()) {
				var instance = edit_form.getForm().getFieldValues();
				model.setSelectProId(instance.projectId);
				// 捕获项目起始周日期设定
				var project_start = -1;
				...
				if (project_start === -1) {
					throw new Error('JS逻辑异常，请联系系统开发人员进行调试。');
				}
				if (project_start === 1) {
					if (instance.startDate.format('w') !== 7) {
			...
			model.loadProbleamsAction(edit_form.getForm().getFieldValues(), resetButtons, n, layout);
		} else if (n === 3 && incr > 0) {
			// 判断问题填写数据的合法性
			var _sign = false;
	...
	function initDefValue() {
		Ext.getCmp('efficiencyExecute').setValue('99990000000000000000000000000002');
		Ext.getCmp('requirementAlter').setValue('99970000000000000000000000000001');
		Ext.getCmp('clientRelation').setValue('99960000000000000000000000000002');
		Ext.getCmp('humanResource').setValue('99980000000000000000000000000001');
	} // end_fun
	...
	function resetUI() {
		...
		Ext.getCmp('projectId').clearValue();
