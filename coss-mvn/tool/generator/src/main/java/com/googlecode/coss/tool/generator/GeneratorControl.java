...
import com.googlecode.coss.tool.generator.util.FileHelper;
import com.googlecode.coss.tool.generator.util.GLogger;
import com.googlecode.coss.tool.generator.util.IOHelper;
import com.googlecode.coss.tool.generator.util.SqlExecutorHelper;
import com.googlecode.coss.tool.generator.util.SystemHelper;
import com.googlecode.coss.tool.generator.util.XMLHelper;
...
public class GeneratorControl {
    private boolean isOverride      = Boolean.parseBoolean(System.getProperty("gg.isOverride",
                                            "false"));
    ...
    public void generateFile(String outputFile, String content, boolean append) {
        try {
            ...
            if (deleteGeneratedFile) {
                GLogger.println("[delete gg.generateFile()] file:" + realOutputFile
                        + " by template:" + getSourceFile());
                new File(realOutputFile).delete();
            } else {
                File file = new File(realOutputFile);
                FileHelper.parnetMkdir(file);
                GLogger.println("[gg.generateFile()] outputFile:" + realOutputFile + " append:"
                        + append + " by template:" + getSourceFile());
                IOHelper.saveFile(file, content, getOutputEncoding(), append);
            }
        } catch (Exception e) {
            GLogger.warn("gg.generateFile() occer error,outputFile:" + outputFile + " caused by:"
                    + e, e);
