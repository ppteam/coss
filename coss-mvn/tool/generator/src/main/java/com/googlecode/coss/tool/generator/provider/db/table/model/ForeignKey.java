...
import com.googlecode.coss.tool.generator.util.ListHashtable;
...
public class ForeignKey {
    ...
    public Column getPkColumn() {
        Column pk = null;
        try {
            Table table = (Table) TableFactory.getInstance().getTable(tableName);
            pk = table.getPkColumn();
        } catch (Exception e) {
            e.printStackTrace();
        }
        return pk;
    ...
    public static class ReferenceKey {
        ...
        public static ReferenceKey fromString(String foreignKey) {
            ...
            String tableSqlName = foreignKey.substring(
                    Math.max(foreignKey.lastIndexOf(".") + 1, 0), foreignKey.indexOf("("));
