package com.googlecode.coss.tool.generator.util.sqlparse;
...
public class SqlParseHelper {
    ...
    static Pattern join   = Pattern
                                  .compile("(join\\s+)(\\w+)(as)?(\\w*)", Pattern.CASE_INSENSITIVE);
    ...
    public static NameWithAlias parseSqlAlias(String str) {
        ...
        if (array.length >= 2) {
            return new NameWithAlias(array[0],
                    StringHelper.tokenizeToStringArray(array[1], " \n\t")[0]);
        }
    ...
    public static class NamedSqlConverter {
        ...
        private String replaceInsertSql2NamedParameters(String sql) {
            if (sql.matches("(?is)\\s*insert\\s+into\\s+\\w+\\s+values\\s*\\(.*\\).*")) {
                if (sql.indexOf("?") >= 0) {
                    throw new IllegalArgumentException("无法解析的insert sql:" + sql
                            + ",values()段不能包含疑问号?");
                } else {
                    return sql;
                }
            ...
            if (m.find()) {
                ...
                if (columns.length != values.length) {
                    throw new IllegalArgumentException("insert 语句的插入列与值列数目不相等,sql:" + sql
                            + " columns:" + StringHelper.join(columns, ",") + " values:"
