...
/**
 * MD5 hash generator. This class takes as input a message of arbitrary length
 ...
 * given pre-specified target message digest. The MD5 algorithm is intended for
...
public class MD5 {
	...
	private int[] decode(byte buffer[], int len, int offset) {
		int i, j;
		for (i = j = 0; j < len; i++, j += 4) {
			decodeBuffer[i] = ((int) (buffer[j + offset] & 0xff)) | (((int) (buffer[j + 1 + offset] & 0xff)) << 8)
					| (((int) (buffer[j + 2 + offset] & 0xff)) << 16) | (((int) (buffer[j + 3 + offset] & 0xff)) << 24);
		}
		return decodeBuffer;
	}
